...
The bundled artwork — the straw hat, the book, and mangame's own M — is
original work generated by `tools/gen_icons.py`. Re-run it with `uv run python
tools/gen_icons.py` after editing the shapes or palettes, or name one emblem to
rebuild just that: `uv run python tools/gen_icons.py mangame`. It needs
`inkscape` and ImageMagick's `convert` on `PATH`.
